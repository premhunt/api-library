...
use Aritic\QueryBuilder\QueryBuilder;
...
class Api implements LoggerAwareInterface
{
    ...
    /**
     * Array of deprecated endpoints to use if a response fails as a 404 with a previous version of Aritic
     *
     * @var array
     */
        ...
        if (!$bcEndpoint || !$this->bcTesting || ($bcEndpoint && $this->bcTesting && $this->bcAttempt)) {
            ...
            if (strpos($url, 'http') === false) {
                $error = array(
                    'code'    => 500,
                    'message' => sprintf(
                        'URL is incomplete.  Please use %s, set the base URL as the third argument to $AriticApi->newApi(), or make $endpoint a complete URL.',
                ...
                try {
                    ...
                    $this->getLogger()->error('Failed connecting to Aritic API: '.$e->getMessage(), array('trace' => $e->getTraceAsString()));
            ...
            if (!empty($error)) {
                ...
                $this->getLogger()->error('Aritic API returned errors: '.var_export($response['errors'], true));
            }
    ...
    /**
     * Returns Aritic version from the HTTP response headers
     * (the header exists since Aritic 2.4.0)
     *
     * @return string|null if not known
     */
    public function getAriticVersion()
    {
        $headers = $this->auth->getResponseHeaders();

        if (isset($headers['Aritic-Version'])) {
            return $headers['Aritic-Version'];
        }
